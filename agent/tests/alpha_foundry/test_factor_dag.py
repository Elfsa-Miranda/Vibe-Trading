...
def _reject_terminal(store: ResearchEventStore, trial_id: str):
    return store.append_event(
        EventDraft(
            ...
            payload={
                ...
                "decision": "reject", "evaluation_event_hash": None, "terminated_at": utc_now_iso(),
            },
        )
    )
...
def _lineage(tmp_path):
    ...
    terminal = _reject_terminal(store, "trial-child")
...
def test_missing_parent_self_edge_cycle_and_forged_depth_are_rejected(tmp_path) -> None:
    ...
    terminal = _reject_terminal(store, "trial-other")
