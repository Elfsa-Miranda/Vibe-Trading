...
def render_alpha_foundry_research_card_markdown(card: AlphaFoundryResearchCard) -> str:
    lines = [
        f"# Alpha Foundry Research Card: {card.card_id}",
        "",
        ...
        f"Factor definition hashes: {', '.join(card.factor_definition_hashes) or 'none'}",
    ...
    if card.proxy_notes:
        ...
        lines.extend(f"- {note}" for note in card.proxy_notes)
    if card.warnings:
        lines.append("")
